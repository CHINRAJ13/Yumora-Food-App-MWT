...
export const registerUser = (data: any) => {
  if (data instanceof FormData) {
    return api.post('/auth/restaurant/register', data, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
  }
  return api.post('/auth/restaurant/register', data);
};
export const loginUser = (data: any) => api.post('/auth/restaurant/login', data);
export const logoutUser = () => api.post('/auth/logout');
export const sendOtp = (phone: string, role?: string) => api.post('/auth/send-otp', { phone, role }); // not used for restaurant usually but keeping structure
